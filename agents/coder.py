...
1. Keep the existing function logic unless specifically asked to change it
2. Make ONLY the changes requested - do not add extra features
3. For interactive programs that loop, use print() to show results, NOT return
4. Handle edge cases as requested (invalid input, try/except, etc.)
5. Do NOT add assertions
6. Do NOT change function signatures unless asked
7. **DOUBLE-CHECK ALL VARIABLE NAMES** - make sure every variable you use is defined!
{interactive_rules}

EXAMPLE - Endless loop with error handling:
```python
def main():
    while True:
        try:
            ...
            if text.lower() == 'quit':
                print("Goodbye!")
                break
            n = int(text)  # Convert the SAME variable you got from input!
            if n < 0:
...
OUTPUT: Return ONLY the complete modified Python code, no explanations.
"""
...
def generate_test_block(func_name: str, params: str, problem: str) -> str:
    ...
    # Prime numbers
    if 'prime' in problem_lower and any(w in problem_lower for w in ['all', 'list', 'display', 'print', 'smaller', 'less', 'under', 'below']):
